// 基于 xterm 4.0.0 `src/terminal_view.dart` 的修补版终端视图。
//
// 修补原因（软键盘输入法兼容性）：
//  1. 原版 `onAction` 仅处理 `TextInputAction.done`。部分输入法（搜狗/讯飞/
//     百度/部分三星键盘等）按回车时不插入 '\n' 字符，而是调用
//     performAction(newline)，被原版丢弃，导致「无法回车执行命令」。
//     终端场景下任何 IME action 的语义都是回车，这里全部映射为 Enter。
//  2. 原版 `keyboardType` 默认 `TextInputType.emailAddress`，与 xterm 内部
//     CustomTextEdit 的 `inputAction: newline` 是非法组合（EditableText 有
//     断言禁止，xterm 自绘输入绕过了断言）：Android 上映射为单行输入框 +
//     IME_ACTION_NONE，各家输入法行为不一。改为 `TextInputType.multiline`
//     后是标准多行组合，绝大多数输入法按回车会插入 '\n' 走 onInsert 路径，
//     与 1 互为双保险。
//
// 除上述两点及类名外，其余逻辑与上游保持逐行一致，便于日后升级 xterm 时对照。
// 上游修复后可删除本文件改回 TerminalView。
//
// 下列 lint 压制均因拷贝上游代码所致：implementation_imports（引用 xterm 内部
// 实现）、invalid_use_of_internal_member（shouldSendPointerInput 为 @internal）、
// sort_child_properties_last / unused_element_parameter（上游代码风格，
// 私有化后部分可选参数未被使用）。
// ignore_for_file: implementation_imports, invalid_use_of_internal_member
// ignore_for_file: sort_child_properties_last, unused_element_parameter
...
class PatchedTerminalView extends StatefulWidget {
  const PatchedTerminalView(
    this.terminal, {
    super.key,
    this.controller,
    ...
    // 修补 2：默认 multiline（原版为 emailAddress，见文件头注释）。
  ...
  /// [TextInputType.multiline] by default (patched, see file header).
  ...
  @override
  State<PatchedTerminalView> createState() => PatchedTerminalViewState();
}

class PatchedTerminalViewState extends State<PatchedTerminalView> {
  ...
  @override
  void didUpdateWidget(PatchedTerminalView oldWidget) {
  ...
  Widget build(BuildContext context) {
    ...
    if (!widget.hardwareKeyboardOnly) {
      child = CustomTextEdit(
        ...
        onAction: (action) {
          _scrollToBottom();
          // 修补 1：不再只认 TextInputAction.done（原版行为）。部分输入法按
          // 回车时走 performAction(newline / go / send / ...) 而非插入 '\n'；
          // 终端场景下任何 IME action 的语义都是回车，全部映射为 Enter。
...
/// 上游 `_TerminalView`（LeafRenderObjectWidget）的原样拷贝，仅改名。
...
/// 上游 `TerminalGestureHandler` 的原样拷贝：它的 `terminalView` 参数类型是
/// 上游私有 State（`TerminalViewState`），无法接收 [PatchedTerminalViewState]，
/// 故一并拷贝并仅改该类型引用，其余逻辑不动。
class _PatchedTerminalGestureHandler extends StatefulWidget {
  const _PatchedTerminalGestureHandler({
  ...
  final PatchedTerminalViewState terminalView;
  ...
  @override
  State<_PatchedTerminalGestureHandler> createState() =>
      _PatchedTerminalGestureHandlerState();
}

class _PatchedTerminalGestureHandlerState
    extends State<_PatchedTerminalGestureHandler> {
  PatchedTerminalViewState get terminalView => widget.terminalView;
