...
import com.venti1112.edgecube.server.KeepAliveManager
...
class MainActivity : FlutterActivity() {
    ...
    override fun configureFlutterEngine(@NonNull flutterEngine: FlutterEngine) {
        ...
        MethodChannel(messenger, powerChannel).setMethodCallHandler { call, result ->
            when (call.method) {
                ...
                "isOverlayEnabled" ->
                    result.success(KeepAliveManager.isOverlayEnabled(applicationContext))
                "setOverlayEnabled" -> {
                    val enabled = call.argument<Boolean>("enabled") ?: false
                    KeepAliveManager.setOverlayEnabled(applicationContext, enabled)
                    result.success(null)
                }
                "canDrawOverlays" ->
                    result.success(KeepAliveManager.canDrawOverlays(applicationContext))
                "requestOverlayPermission" -> {
                    requestOverlayPermission()
                    result.success(null)
                }
                "getOverlayOptions" ->
                    result.success(KeepAliveManager.overlayOptions(applicationContext).toMap())
                "setOverlayOptions" -> {
                    val map = call.arguments as? Map<*, *>
                    if (map == null) {
                        result.error("BAD_ARGS", "缺少悬浮窗设置", null)
                    } else {
                        KeepAliveManager.setOverlayOptions(applicationContext, map)
                        result.success(null)
                    }
                }
                // —— 返回键退出策略 ——
    ...
    private fun requestOverlayPermission() {
        if (KeepAliveManager.canDrawOverlays(applicationContext)) return
        try {
            startActivity(
                Intent(
    ...
    private fun getSystemInfo(serverManager: ServerProcessManager): Map<String, Any?> {
        val memInfo = SystemStats.memInfo(this)
