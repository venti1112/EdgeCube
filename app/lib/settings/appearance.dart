...
/// 单项设置 Notifier 的公共骨架:把"读哪一项 / 写哪一项"缩成两个方法。
abstract class SettingNotifier<T> extends Notifier<T> {
  /// 从聚合状态里取出本项的值
  T selectValue(AppearanceSettings s);

  /// 把本项的新值交给聚合状态(由它负责 clamp 与落盘)
  Future<void> applyValue(AppearanceSettingsNotifier notifier, T value);

  @override
  T build() => ref.watch(appearanceSettingsProvider.select(selectValue));

  /// 写入本项设置
  Future<void> set(T value) =>
      applyValue(ref.read(appearanceSettingsProvider.notifier), value);
}
