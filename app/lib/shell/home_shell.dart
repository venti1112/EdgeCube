...
class HomeShell extends ConsumerWidget {
  ...
  ///
  /// 未连接时在顶部插一条连接横幅:掉线不把人踢回连接页,
  /// 各分支页面状态照常保留(见 connection_banner.dart)。
  Widget _contentArea(BuildContext context, WidgetRef ref, bool glass) {
    Widget child = navigationShell;
    if (ref.watch(connectionPhaseProvider) != ConnectionPhase.connected) {
      child = Column(
        children: [_banner(ref), Expanded(child: child)],
      );
    }
  ...
  /// 连接横幅(毛玻璃模式下按内容表面不透明度融合,避免突兀的色块)
  Widget _banner(WidgetRef ref) {
    if (!ref.watch(glassModeEnabledProvider)) return const ConnectionBanner();
    final opacity = ref.watch(contentOpacityProvider);
    return Opacity(
