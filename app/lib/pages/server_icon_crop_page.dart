...
/// 服务器图标裁剪页(移植自 V1):将导入的图片裁剪为 1:1 并缩放至 64×64,
/// 输出 PNG 字节。用户确认裁剪后以 `Navigator.pop(Uint8List)` 返回结果,
/// 取消返回 `null`。刻意固定为黑底白字(与相册取景一致),不随明暗主题变化。
class ServerIconCropPage extends StatefulWidget {
  const ServerIconCropPage({super.key, required this.imageBytes});
