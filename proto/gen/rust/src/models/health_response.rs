/*
 * EdgeCube Daemon API
 *
 * EdgeCube v2 守护进程 API(唯一契约)。  - 契约优先:所有 API 变更先改本文件,再由生成器产出三端代码   (Rust daemon / Kotlin daemon / Flutter client),任何一端不得绕过契约私自改协议。 - 实现阶段标记:每个 path 的 `x-phase` 标注落地阶段(0/1/2/3),对应   docs/PLAN.md §10 分阶段路线。 - WS 部分(events / terminal)不在 OpenAPI 能力范围内,见同目录 `ws.md`。 - 鉴权:除 /auth/_* 与 /health 外全部需要 `Authorization: Bearer <token>`;   6 位对码经 `auth/pair` 换取长期 token。 
 *
 * The version of the OpenAPI document: 0.1.0
 * 
 * Generated by: https://openapi-generator.tech
 */

use crate::models;
use serde::{Deserialize, Serialize};

#[derive(Clone, Default, Debug, PartialEq, Serialize, Deserialize)]
pub struct HealthResponse {
    #[serde(rename = "status")]
    pub status: Status,
    /// daemon 版本
    #[serde(rename = "version")]
    pub version: String,
    #[serde(rename = "daemon")]
    pub daemon: Daemon,
    #[serde(rename = "platform")]
    pub platform: String,
    #[serde(rename = "uptimeSeconds")]
    pub uptime_seconds: i64,
    #[serde(rename = "instances", skip_serializing_if = "Option::is_none")]
    pub instances: Option<Box<models::HealthResponseInstances>>,
}

impl HealthResponse {
    pub fn new(status: Status, version: String, daemon: Daemon, platform: String, uptime_seconds: i64) -> HealthResponse {
        HealthResponse {
            status,
            version,
            daemon,
            platform,
            uptime_seconds,
            instances: None,
        }
    }
}
/// 
#[derive(Clone, Copy, Debug, Eq, PartialEq, Ord, PartialOrd, Hash, Serialize, Deserialize)]
pub enum Status {
    #[serde(rename = "ok")]
    Ok,
    #[serde(rename = "degraded")]
    Degraded,
}

impl Default for Status {
    fn default() -> Status {
        Self::Ok
    }
}
/// 
#[derive(Clone, Copy, Debug, Eq, PartialEq, Ord, PartialOrd, Hash, Serialize, Deserialize)]
pub enum Daemon {
    #[serde(rename = "rust")]
    Rust,
    #[serde(rename = "kotlin")]
    Kotlin,
}

impl Default for Daemon {
    fn default() -> Daemon {
        Self::Rust
    }
}

