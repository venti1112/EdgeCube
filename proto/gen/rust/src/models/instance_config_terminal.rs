/*
 * EdgeCube Daemon API
 *
 * EdgeCube v2 守护进程 API(唯一契约)。  - 契约优先:所有 API 变更先改本文件,再由生成器产出双端代码   (Rust daemon / Flutter client),任何一端不得绕过契约私自改协议。 - 实现阶段标记:每个 path 的 `x-phase` 标注落地阶段(0/1/2/3),对应   docs/PLAN.md §10 分阶段路线。 - WS 部分(events / terminal)不在 OpenAPI 能力范围内,见同目录 `ws.md`。 - 鉴权:除 /auth/_* 与 /health 外全部需要 `Authorization: Bearer <token>`;   6 位对码经 `auth/pair` 换取长期 token。 
 *
 * The version of the OpenAPI document: 0.1.0
 * 
 * Generated by: https://openapi-generator.tech
 */

use crate::models;
use serde::{Deserialize, Serialize};

#[derive(Clone, Default, Debug, PartialEq, Serialize, Deserialize)]
pub struct InstanceConfigTerminal {
    #[serde(rename = "pty", skip_serializing_if = "Option::is_none")]
    pub pty: Option<bool>,
    #[serde(rename = "initialCols", skip_serializing_if = "Option::is_none")]
    pub initial_cols: Option<i32>,
    #[serde(rename = "initialRows", skip_serializing_if = "Option::is_none")]
    pub initial_rows: Option<i32>,
    /// 是否把 Minecraft §x 色码转 ANSI
    #[serde(rename = "haveColor", skip_serializing_if = "Option::is_none")]
    pub have_color: Option<bool>,
}

impl InstanceConfigTerminal {
    pub fn new() -> InstanceConfigTerminal {
        InstanceConfigTerminal {
            pty: None,
            initial_cols: None,
            initial_rows: None,
            have_color: None,
        }
    }
}

